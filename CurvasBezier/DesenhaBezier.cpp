...
void init()
{
    ...
    CarregaModelos(); //daqui que vem a meia seta
...
void DesenhaCurvas()
{
    for(int i=0; i<nCurvas;i++)
    {
        ...
        //nCurvas++; //isto eh o atualizar curvas???
    }
}
// **********************************************************************
//
...
{
    glPushMatrix();
        //glTranslated(EnvMin[i].x, EnvMin[i].y, EnvMin[i].z); //utilizando os envmin
        glTranslated(11,-15,0); // veja o arquivo MeiaSeta.txt //minimo px, min py, min pz 
        //ter uma funcao q cria os nossos menus
...
void Mouse(int button,int state,int x,int y)
{
    ...
    FoiClicado = true;
    if (nPontoAtual < 3) {
        PontosClicados[nPontoAtual] = PontoClicado;
        nPontoAtual++;      
        cout << "clicou no ponto em (" << ox << "," << oy << "," << oz << ")" << endl;
    } 
    else {
        nPontoAtual = 0;
        CriaCurvas();
